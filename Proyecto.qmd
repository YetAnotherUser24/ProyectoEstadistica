...
```{r eval=TRUE}
ITS %>% group_by(Año,Mes,Aeropuerto) %>% summarise(IT = sum(`Importe por Servicios`, na.rm = T)) -> DF
...
```{r}
IRS %>% group_by(Año,Mes,Aeropuerto) %>% summarise(IR = sum(`Importe por Servicios`, na.rm = T)) -> IRegulados
